...
def val_flow(sess, model, init, input_list, test_iter):
    ...
    for i in range(1, TEST_NUM + 1):
        ...
        # flow Stream
        model.load_state_dict(torch.load(os.path.join(ckpt['flow'], 'flow_' + str(test_iter) + '.pth'))["model"])
        model.eval()


        flow_class_result, flow_attention, flow_tCam, \
        erased_outputs, erased_tcam, erased_att, \
        erased2_outputs, erased2_tcam, erased2_att = model(flow_features, "")
        ...
        flow_attention = flow_attention.cpu().data.numpy()
        erased_att = erased_att.cpu().data.numpy()
        fusion_attention = np.maximum(flow_attention, erased_att)

        # -------- test ------------------------
        sig_flow = sigmoid(flow_tCam)
        sig_era = sigmoid(erased_tcam)
        sig_att_flow = flow_attention * sig_flow
        ...
        if flow_class_prediction.any():
            # Weighted T-CAM
            flow_wtCam = get_wtCAM(flow_tCam, "", flow_attention, ALPHA, flow_class_prediction)
